'''
Plot neutrino oscillations with sidereal SME parameters activated
...
if __name__ == "__main__" :
    ...
    E_GeV = np.geomspace(1., 1e4, num=100)
    coszen = -1.
    ...
    kw = {}
    if solver == "nusquids" :
        kw["energy_nodes_GeV"] = np.geomspace(E_GeV[0], E_GeV[-1], num=10)
    ...
    flavor_structure = np.array([0., 0., 1.]) # not Equally shared between flavors (diagonal)
    ...
    for case_label, (a_eV) in cases.items() :
        ...
        # Loop over RA
        P_std, P_liv = np.zeros(len(ra_values_deg)), np.zeros(len(ra_values_deg))
        for i in range(len(ra_values_deg)) :

            # Units
            ra_deg = ra_values_deg[i]
            ra_rad = np.deg2rad(ra_values_deg[i])
            ...
            P_std[i] = np.squeeze( calculator.calc_osc_prob(**calc_kw)[...,final_flavor] ) # Single value

            # Get LIV osc probs
            calculator.set_sme(directional=directional, basis=sme_basis, a_eV=a_eV, c=null_operator, e=null_operator)
            P_liv[i] = np.squeeze( calculator.calc_osc_prob(**calc_kw)[...,final_flavor] ) # Single value
