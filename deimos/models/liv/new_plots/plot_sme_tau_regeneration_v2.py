...
if __name__ == "__main__":
    ...
    # initial_flavor = (0,1,2)  # 1 corresponds to numu
    final_flavor = 2
    ...
    initial_flavors = (0,1,2) 
    for i, initial_flavor in enumerate(initial_flavors):
        if initial_flavor == 0: label = r"$\nu_e$"
        elif initial_flavor == 1: label = r"$\nu_\mu$"
        elif initial_flavor == 2: label = r"$\nu_\tau$"
        ...
        ax.plot(E_values_GeV, flux_ratios[:,2], linestyle="-", label=label)
